...
class VersionDetector {
  ...
  checkGPUQuality() {
    try {
      ...
      const isIntegrated = integratedPatterns.some((pattern) =>
        pattern.test(renderer),
      );

      // Consider discrete GPUs as good (NVIDIA GeForce, AMD Radeon, etc.)
      const isDiscrete =
        /nvidia|geforce|radeon|amd|ati/i.test(renderer) && !isIntegrated;
  ...
  async runPerformanceTest() {
    return new Promise((resolve) => {
      try {
        ...
        const totalParticles = starLayers.reduce(
        ...
        const animate = () => {
          ...
          if (elapsed < testDuration) {
            ...
            console.log(
              `Performance test: ${fps.toFixed(1)} FPS with ${totalParticles} particles (exact real site load)`,
            );
  ...
  loadAndExecuteBaseScript() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch base script: ${response.status} ${response.statusText} `,
          );
        }
        ...
        const executableScript = wrappedScript.replace(
          /}\s*\)\s*;?\s*$/,
          "})();",
        );
  ...
  loadAndExecuteRetroScript() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch retro script: ${response.status} ${response.statusText} `,
          );
        }
        const scriptText = await response.text();
  ...
  addVersionToggle(currentVersion) {
    ...
    const createButton = (vName) => {
      ...
      btn.style.background =
        vName === currentVersion ? "#FF0000" : "transparent";
      btn.style.color = "#FFF";
    ...
    autoBtn.style.background = !window.location.search.includes("v=")
      ? "#FF0000"
      : "transparent";
