document.addEventListener("DOMContentLoaded", () => {
  ...
  const spaceObjectContainer = document.getElementById(
  ...
  function updateResponsiveSettings() {
    ...
    if (
      state.starSizeScale !== state._prevStarSizeScale &&
      state.stars.length > 0
    ) {
      const ratio = state.starSizeScale / (state._prevStarSizeScale || 1);
      state.stars.forEach((s) => {
        s.size = Math.max(0.2, s.size * ratio);
      });
  ...
  function getStarMovementRatio(star) {
    ...
    if (depth < 0.33) return 0.06;
    if (depth < 0.66) return 0.04;
    return 0.02;
  }
  ...
  function brightenRgba(rgbaStr, targetAlpha) {
    ...
    if (!parts || parts.length < 3)
      return `rgba(255, 255, 255, ${targetAlpha})`;
  ...
  class Star {
    constructor() {
      ...
      this.size = Math.floor((Math.random() * 2 + 1) * state.starSizeScale) * 2; // blockier size
      ...
      if (this.isRedGiant) {
        ...
        this.size =
          Math.floor((Math.random() * 2 + 2) * state.starSizeScale) * 2;
    ...
    draw(ctx, pX, pY) {
      ...
      if (this.isShining) {
        ...
        const sineProgress = Math.sin(
          (this.shineProgress / this.shineDuration) * Math.PI,
        );
        ...
        const drawShine = (alpha, length, angleOffset) => {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            ctx.lineTo(
              parallaxX + Math.cos(angle) * crossLength,
              parallaxY + Math.sin(angle) * crossLength,
            );
            ctx.stroke();
          }
        };
      ...
      if (this.isRedGiant) {
        ...
        currentOpacity =
          this.baseOpacity +
          Math.sin(
            Date.now() * 0.001 * this.twinkleSpeed + this.twinkleOffset,
          ) *
          0.3;
      }
      ...
      ctx.fillRect(
        Math.floor(parallaxX / 2) * 2,
        Math.floor(parallaxY / 2) * 2,
        this.size,
        this.size,
      );
    }
  ...
  class ShootingStar {
    constructor() {
      ...
      const startX =
        Math.random() * starsCanvas.width * 1.5 - starsCanvas.width * 0.25;
      ...
      this.opacity = 1;
      const colorSet =
        config.shootingStarColors[
        Math.floor(Math.random() * config.shootingStarColors.length)
        ];
    ...
    isOffscreen() {
      return (
        this.x < -this.len ||
        this.y > starsCanvas.height + this.len ||
        this.opacity <= 0
      );
    }
  }
  ...
  function updateMousePosition() {
    ...
    const smoothingFactor =
      state.isMouseIdle || state.isMouseOutside ? 0.02 : 0.05;
    ...
    if (
      currentTime - state.lastMouseActivityTime > 3000 &&
      !state.isMouseIdle &&
      !state.isMouseOutside
    ) {
  ...
  function drawStars() {
    ...
    state.stars.forEach((star) =>
      star.draw(starsCtx, combinedMouseX, combinedMouseY),
    );
  }
  ...
  function selectSpaceObject() {
    const selected =
      config.spaceObjects[
      Math.floor(Math.random() * config.spaceObjects.length)
      ];
    const element = document.getElementById(selected.id);
    document
      .querySelectorAll(".space-object")
  ...
  function launchSpaceObject() {
    ...
    const size =
      spaceObject.element && spaceObject.element.offsetWidth
        ? spaceObject.element.offsetWidth
        : spaceObject.element
          ? Math.round(spaceObject.element.getBoundingClientRect().width)
          : 38;
